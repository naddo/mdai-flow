...
class MDAIExplorer:
    ...
    def load_config_and_data(self, config_path):
        ...
        for d_id in d_ids:
            try:
                ...
                self.silence()
                try:
                    p = client.project(self.project_id, dataset_id=d_id)
    ...
    def on_item_select(self, e):
        ...
        sop_uid, ds_id = row.get('SOPInstanceUID'), row.get('datasetId')
        path = os.path.join(self.cache_dir, f"{sop_uid}.dcm")
        if not os.path.exists(path):
            self.status.config(text="⏳ Fetching DICOM...")
            self.root.update_idletasks()
            try:
                self.silence()
                try:
                    self.project_instances[ds_id].download_dicom(sop_uid, output_dir=self.cache_dir)
                finally:
                    self.unsilence()
            except: 
                messagebox.showerror("Error", "Role 'User' cannot download pixels.")
                return
